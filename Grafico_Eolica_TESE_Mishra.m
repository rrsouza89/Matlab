...
%         kE4 = 3.07;  c4 = 7.48;
        ...
        fw2 = (-(kE2.*exp(-((vi2.*wr - w.*(vi2 - vr2))/(c2.*wr)).^kE2).*(vi2 - vr2).*((vi2.*wr - w.*(vi2 - vr2))/(c2.*wr)).^(kE2 - 1))/(c2.*wr));
        wr2 = exp(-(vr/c2).^kE2) - exp(-(v0/c2).^kE2); 
        
        %w3         
        w03 = 1 - exp(-(vi3/c3).^kE3) + exp(-(v0/c3).^kE3);
        fw3 = (-(kE3.*exp(-((vi3.*wr - w.*(vi3 - vr3))/(c3.*wr)).^kE3).*(vi3 - vr3).*((vi3.*wr - w.*(vi3 - vr3))/(c3.*wr)).^(kE3 - 1))/(c3.*wr));
        wr3 = exp(-(vr/c3).^kE3) - exp(-(v0/c3).^kE3); 
        
        %w4
        w04 = 1 - exp(-(vi4/c4).^kE4) + exp(-(v0/c4).^kE4);
        fw4 = (-(kE4.*exp(-((vi4.*wr - w.*(vi4 - vr4))/(c4.*wr)).^kE4).*(vi4 - vr4).*((vi4.*wr - w.*(vi4 - vr4))/(c4.*wr)).^(kE4 - 1))/(c4.*wr));
        wr4 = exp(-(vr/c4).^kE4) - exp(-(v0/c4).^kE4); 
